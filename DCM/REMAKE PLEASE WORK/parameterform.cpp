...
{
    // settings_ is mutable (QSettings is not logically const)
...
{
    // Example rule: amplitude must be a multiple of 0.5 V between 0 and 7.5 V
    if (v < 0.0 || v > 7.5) {
...
bool ParameterForm::validate(QString* err) const
{
    ...
    // You can add any other spec-specific checks here
...
void ParameterForm::reflectValidity()
{
    ...
    // Right now we only emit a status message; you can also colour labels if you like.
    emit statusMessage(ok ? tr("Parameters are valid.") : err);
}
...
void ParameterForm::onSave()
{
    ...
    if (!tryBuildProfile(&p, &err)) {
        emit statusMessage("Cannot save: " + err);
        return;
    }

    QString dbErr;
    if (!Database::upsertProfile(p, &dbErr)) {
        emit statusMessage("DB error: " + dbErr);
        return;
    }
...
void ParameterForm::onLoad()
{
    ...
    if (!opt) {
        emit statusMessage(dbErr.isEmpty()
                           ? QString("No saved profile for mode %1").arg(m)
                           : "DB error: " + dbErr);
        return;
    }
...
// Matches typical lab mapping: 0..7 for AOO,VOO,AAI,VVI,AOOR,VOOR,AAIR,VVIR
...
// ---------- frame encode / decode ----------
//
// Layout matches your Simulink screenshot (1-based indices):
//   1   (unused / header)        [we send 0]
//   2   command / reserved       [we send 1 = SET_PARAM]
//   3   Mode                     (1 byte)
//   4   LRL                      (1 byte)
//   5   MSR/URL                  (1 byte)
//   6-9   AAmp  (float, 4 bytes, little-endian)
//   10-13 VAmp  (float)
//   14-17 APW   (float)
//   18-21 VPW   (float)
//   22-23 VRP   (uint16, little-endian)
//   24-25 ARP   (uint16)
//   26    HysteresisTime (1 byte)       [we send 0]
//   27-28 AVD  (uint16)                 [we send 0]
//   29    ReactionTime (1 byte)         [we send 0]
//   30    ResponseFactor (1 byte)       [we send 0]
//   31    RecoveryTime (1 byte)         [we send 0]
//   32    unused                        [0]
...
bool ParameterForm::buildTxFrame(QByteArray* outFrame, QString* errMsg) const
{
    ...
    frame[0] = 0;       // reserved
    frame[1] = 1;       // "SET_PARAM" command
    frame[2] = char(modeToCode(p.mode));
    frame[3] = char(p.lrl.value_or(60));
    frame[4] = char(p.url.value_or(120));
    ...
    // 1-based indices 6..9,10..13,14..17,18..21 -> 0-based 5,9,13,17
    putFloat(5,  p.aAmp.value_or(3.5));
    putFloat(9,  p.vAmp.value_or(3.5));
    putFloat(13, p.aPw.value_or(0.4));
    putFloat(17, p.vPw.value_or(0.4));

    // VRP at 22-23, ARP at 24-25  -> 0-based 21,23
    putU16(21, p.vrp.value_or(320));
    putU16(23, p.arp.value_or(250));

    // HysteresisTime (26), AVD (27-28), Reaction, Response, Recovery defaults
    frame[25] = 0;          // Hysteresis
    putU16(26, 0);          // AVD
